...
@Component({
  ...
  styleUrl: './product-sale-report.component.scss'
})
export class ProductSaleReportComponent {
  ...
  records: any[] = [];
  fromDate: Date = new Date(new Date().getFullYear(), new Date().getMonth(), 1); // 1st of current month
  endDate: Date = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0); // last day of current month
  ...
  onGenerate() {
    this._ReportService
      .getProductSaleReport(this.fromDate, this.endDate)
      .subscribe({
        next: (res: any) => {
  ...
  async onPrintReport() {
    ...
    this._pdfService.print(docDefinition, 'Product Sale Report.pdf');
  }
  ...
  async onPdfGenerate() {
    ...
    this._pdfService.generatePdf(docDefinition, 'Product Sale.pdf');
  }
            ...
            body: [
              ...
              ...element.items.map((ed: any, index: number) => {
                return [
                  {
                    text: ed?.ItemName
                      ? ed?.ItemName.toString().toLocaleUpperCase()
                      : '',
                    style: 'TableFontSize',
                  },
                  {
